    ...
    >
      ...
      >
        ...
        <el-date-picker
          ...
          type="datetime"
          :picker-options="
            item.pickerOptions ? item.pickerOptions : setOptions(item, index)
          "
          placeholder="选择时间"
          :format="item.dateFormat ? item.dateFormat : 'yyyy-MM-dd HH:mm:ss'"
          :value-format="
      ...
      <slot name="last"></slot>
      <!-- 查询按钮 -->
      <el-button type="primary" class="scpp-button" @click="sendSearch"
        >搜索
      </el-button>
      <el-button
      ...
      <span
        ...
        <i
          :class="[
            showCondition ? 'el-icon-arrow-down' : 'el-icon-arrow-up',
            'arrow-icon',
          ]"
      /></span>
    </el-form>
    ...
    <vab-query-form v-if="TopAction && TopAction.isTopAction">
      <vab-query-form-left-panel>
        <template v-for="(item, index) in TopAction.left">
          <el-button
            ...
            @click="(e) => handleTopAction(item.label, e)"
          >
            {{ item.label }}
          </el-button>
        </template>
      ...
      <vab-query-form-right-panel>
        <template v-for="(item, index) in TopAction.right">
          <el-dropdown v-if="item.dropdown" trigger="click" placement="bottom">
...
export default {
  ...
  mounted() {
    this.$refs.draggable_tree ? this.$refs.draggable_tree[0].filter() : ''
  },
